    ...
    conversationWith: {
      ...
      resolve: (user, args) => {
        return messages.filter(message => {
          return (
            (message.senderId === args.partnerId &&
...
const ClassType = new GraphQLObjectType({
  name: 'Class',
  description:
    'This represents a single class session that is requested, scheduled, or attended',
  fields: () => ({
    id: {type: GraphQLNonNull(GraphQLInt)},
    ...
    learner: {
      type: UserType,
      resolve: item => {
        return users.find(user => user.id === item.learnerId); // add database info
      },
    },
    skill: {
      type: SkillType,
    ...
    updateSkill: {
      ...
      resolve: (parent, args) => {
        ...
        if (args.availability) {
          skill.availability = args.availability;
        }
        if (args.duration) {
          skill.duration = args.duration;
        }
        return skill;
      },
    },
